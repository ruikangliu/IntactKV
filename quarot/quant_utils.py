...
class ActQuantizer(torch.nn.Module):
    ...
    def forward(self, x):
        x_dtype = x.dtype
        if self.bits == 16 or (not self.activate_quantizer):
            return x
        elif self.sym:
    ...
    def quantize(self, x):
        if not self.activate_quantizer:
            return x
        if self.sym:
...
class WeightQuantizer(torch.nn.Module):
    ...
    def quantize(self, x):
        x_dtype = x.dtype
        if self.ready() and self.bits < 16 and self.activate_quantizer:
            if self.sym:
                return sym_quant_dequant(x, self.scale, self.maxq).to(x_dtype)
            return asym_quant_dequant(x, self.scale, self.zero, self.maxq).to(x_dtype)
        return x
...
def set_quantizer_state(model, activate_quantizer=True):
    for m in model.modules():
        if isinstance(m, (WeightQuantizer, ActQuantizer)):
            m.activate_quantizer = activate_quantizer
    return model
